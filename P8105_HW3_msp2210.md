...
This plot depicts the mean activity per minute against minute to show
the per day activity-time course, stratified by educational level and
divided by gender. I chose to look at mean data instead of
individual-level data as it makes group-level trends more obvious, and
reduces a lot of the noise that resulted from when I initially created
the plot with individual data (I went back and revised my plot to
include mean activity). Based on this, the mean activities/minute for
men and women both majorly overlap for all three educational categories,
with women having slightly increased mean activity throughout. When
comparing educational levels, those in the `less than high school`
category had greater overall mean activity/minute than those in the
